...
function ItemFollow(props, {hand}){
    ...
    const handleClicUnFollow = (e) =>{
        ...
        const fetchDelete = async () =>{
            const res = await fetch(`https://${import.meta.env.VITE_DIR_IP}/delete-follow?IDOrigen=${user.IDUsuario}&IDDes=${idUser}`, {method: 'DELETE'});
            if(res.ok){
    ...
    return(
        <div key={nombre} className={styles.container}>
            <div className={styles.imgContainer}>
                {
                    foto ? 
                    <img src={`https://${import.meta.env.VITE_DIR_IPP}/` + foto + ".png"} alt="Foto de perfil" className={styles.img} />  //asdsa
                    :
