...
function Lista({ idUser }) {
    ...
    const getLists = () => {
        const cons = async () => {
            const res = await fetch(`https://${import.meta.env.VITE_DIR_IP}/get-listas?IDUsuario=${idUser}`, { method: 'POST' });
    ...
    const handleDeleteList = (listID, IDImangen) => {
        const fetchDeleteList = async () => {
            await fetch(`https://${import.meta.env.VITE_DIR_IP}/eliminar-lista?id=${listID}`, { method: 'DELETE' });
        }
        fetchDeleteList();
        const fetchDeleteImage = async () => {
            await fetch(`https://${import.meta.env.VITE_DIR_IPP}/image/delete/${IDImangen}`, { method: 'DELETE' });
        }
        fetchDeleteImage();
