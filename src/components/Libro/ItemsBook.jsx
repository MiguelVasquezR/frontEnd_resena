...
function ItemsBook({libro}) {    
    ...
    useEffect(() => {
        const nameA = async () => {            
            const res = await fetch(`http://${import.meta.env.VITE_DIR_IP}/nombre-autores?id=${IDAutor}`, {method: 'GET'});            
    ...
    return (
        <div key={titulo + IDAutor} className={styles.bookContainer} onClick={handleSeeBook}>
            ...
            <div className={styles.izquierdo}>
                <img src={`http://${import.meta.env.VITE_DIR_IPP}/`+ foto + ".png"} alt={foto} className={styles.img} />                        
            </div>
