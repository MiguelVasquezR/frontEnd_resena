...
function PhotoPerfil({band}) {    
    ...
    const handleUploadImage = (e) =>{
        ...
        const fetchUploadImage = async () =>{            
            const res = await fetch(`https://${import.meta.env.VITE_DIR_IPP}/image/post`, {method: 'POST', body: formData});                        
    ...
    const handleSavedImage = () => {
        const user = getUser();                
        const han = async () => {
            const res = await fetch(`https://${import.meta.env.VITE_DIR_IP}/foto-usuario?IDUsuario=${user.IDUsuario}&Foto=${localStorage.getItem('IDImagen')}`, {method: 'PUT'});
            if  (res.ok){
