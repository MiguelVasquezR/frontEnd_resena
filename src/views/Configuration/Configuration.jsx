...
function Configuration() {
    ...
    const handleAddRed = (e) => {
        ...
        const fetchAddRed = async () => {
            try {
                ...
                const res = await fetch(`https://${import.meta.env.VITE_DIR_IP}/red-social`, { method: 'POST', body: JSON.stringify(datos) });
            } catch (error) {
                console.log(error);
            }
        }
    ...
    const GetRedSocial = () => {
        const fetchGetRed = async () => {
            const user = getUser();
            const res = await axios.get(`https://${import.meta.env.VITE_DIR_IP}/red-social`, { params: user });
            setRedes(res.data);
            setOpen(false);
        }
        fetchGetRed();
                    ...
                    <div>
                        ...
                        <div className={styles.redesContainer}>
                            ...
                            <div className={styles.optionRedesContainer}>
                                {redes ? <div className={styles.redesOptionGet}>{redes.map((red, i) => { return <a target="_blank" key={red.URL +"--"+ i} href={red.URL} className={styles.urls}> {selectLogo(red.URL)} </a> })}</div> : ''}
