...
function OptionsBooks({ IDLibro, titulo, foto, idioma, editorial, IDAutor }) {
    ...
    const handleAddBook = () => {
        ...
        if (add) {
            fetch(`https://${import.meta.env.VITE_DIR_IP}/agregar-libros?idlista=${id.ID}&idLibro=${IDLibro}`, { method: 'POST' }).then(respuesta => { if (respuesta.ok) { console.log("Se ha agregado"); } }).catch(err => { console.log("Error al momento de hacer la petición"); })
        } else {
            fetch(`https://${import.meta.env.VITE_DIR_IP}/eliminar-libros?idlista=${id.ID}&idLibro=${IDLibro}`, { method: 'DELETE' }).then(respuesta => { }).catch(err => { })
        }
