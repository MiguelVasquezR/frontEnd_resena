...
function OptionsBooks({IDLibro, titulo, foto, idioma, editorial, IDAutor}){
    ...
    const handleAddBook = () =>{
        ...
        if (add){
            fetch(`http://192.168.1.67:4567/agregar-libros?idlista=${id.ID}&idLibro=${IDLibro}`, {method: 'POST'}).then(respuesta => {if(respuesta.ok) {console.log("Se ha agregado");}}).catch(err => {console.log("Error al momento de hacer la petición");})
        }else{
            fetch(`http://192.168.1.67:4567/eliminar-libros?idlista=${id.ID}&idLibro=${IDLibro}`, {method: 'DELETE'}).then(respuesta => {}).catch(err => {})
        }
    ...
    const getAutor = () =>{
        const fetchAutor = async () =>{
            const res = await fetch(`http://192.168.1.67:4567/nombre-autores?id=${IDAutor}`);
    ...
    return(
        <div className={styles.container}>
            <div className={styles.containerImg}>
                <img src={'http://192.168.1.67:9000/' + foto + '.png'} alt="Portada Libro" className={styles.img}/>
            </div>
